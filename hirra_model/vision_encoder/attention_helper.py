...
class Attention(nn.Module):
    ...
    def forward(
        ...
        # device đã được lấy từ x.device ở trên rồi
        if exists(context):
        ...
        if self.causal:
            ...
            # device đã được định nghĩa ở trên từ x.device
...
class AlibiPositionalBias(nn.Module):
    ...
    def get_bias(self, i, j, device):
        # device được truyền vào từ bên ngoài
    ...
    def forward(self, sim):
        ...
        # device đã được lấy từ sim.device ở trên
...
class ContinuousPositionBias(nn.Module):
    ...
    def forward(self, *dimensions, device = None):
        # Nếu không truyền device vào, sử dụng device mặc định của model
