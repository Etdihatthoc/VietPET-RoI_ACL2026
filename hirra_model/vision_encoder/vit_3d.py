...
from .attention_helper import Attention, Transformer, ContinuousPositionBias
...
def gradient_penalty(images, output, weight = 10):
    ...
    device = images.device  # Lấy device từ input tensor
...
def pick_video_frame(video, frame_indices):
    batch = video.shape[0]
    device = video.device  # Lấy device từ input tensor
...
class CTViT(nn.Module):
    ...
    def copy_for_eval(self):
        device = next(self.parameters()).device  # Lấy device từ model parameters
    ...
    def encode_features(self, video):
        """
        Mục đích: Chỉ chạy phần encoder của VAE (patching + transformers)
                để trích xuất đặc trưng 3D (tokens). Bỏ qua VQ và Decoder.
                Đây là hàm chúng ta sẽ gọi từ MultimodalEncoder.

        Đầu vào:
            - video (torch.Tensor): Ảnh 3D đầu vào (đã được resize).
            Shape: [B, C, F, H, W]
            Ví dụ: [B, 1, 201, 128, 128]

        Đầu ra:
            - tokens (torch.Tensor): Các token đặc trưng 3D.
            Shape: [B, T, H_patch, W_patch, D]
            Ví dụ: [B, 101, 8, 8, 512]
            (B: Batch, T: Số patch thời gian, H_patch, W_patch: Số patch
            không gian, D: Chiều đặc trưng)
        """

        # Logic này được sao chép từ hàm `forward` của CTViT gốc
        #
        first_frame, rest_frames = video[:, :, :1], video[:, :, 1:]

        # 1. Patching và Embedding
        # Chuyển ảnh thành các patch token
        ...
        # 2. Chạy qua các Transformer Encoder
        # Hàm `encode` này bên trong CTViT chính là tổ hợp của
        # enc_spatial_transformer và enc_temporal_transformer
        #
        tokens = self.encode(tokens)

        # 3. Trả về đặc trưng
        # Chúng ta DỪNG LẠI TẠI ĐÂY (ngay trước bước self.vq)
        # và trả về các token đặc trưng.
        return tokens
