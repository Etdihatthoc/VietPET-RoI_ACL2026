"""
Stage 4 Training Script: End-to-End LLM Finetuning (LoRA)

Trains LoRA adapters on language decoder:
- LoRA adapters (q_proj, k_proj, v_proj, o_proj, etc.)
- Optionally: embed_tokens and lm_head

All other components (vision encoder, feature extractor, projectors) are frozen.
"""
...
# Import from stage4 modules
from data import Stage3Dataset, Stage3Collator  # Reuse Stage 3 dataset
from models import build_stage4_model           # NEW: Stage 4 model builder
from engine import Stage3Trainer                # Reuse Stage 3 trainer
from utils import (
    setup_logger,
    init_wandb,
    set_seed,
    get_device,
    build_optimizer,                            # NEW: Stage 4 optimizer
    build_scheduler,
)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Stage 4 Training - End-to-End LLM Finetuning (LoRA)"
    )
    parser.add_argument(
        "--config",
        type=str,
        default="config_stage4.yaml",
...
def build_dataloaders(config: dict, tokenizer) -> tuple:
    ...
    # Create datasets (reuse Stage3Dataset)
    ...
    # Create collator (reuse Stage3Collator)
...
def main():
    ...
    # Setup logger
    logger = setup_logger(output_cfg.get("logs_dir", "outputs/logs"), name="stage4")
    logger.info("=" * 80)
    logger.info("STAGE 4 TRAINING: END-TO-END LLM FINETUNING (LoRA)")
    logger.info("=" * 80)
    ...
    logger.info("=" * 80)
    model, build_info = build_stage4_model(config)

    # Log model info
    ckpt_meta = build_info.get("checkpoint", {})
    if ckpt_meta:
        logger.info(f"Loaded Stage 3 checkpoint: {ckpt_meta.get('checkpoint')}")
        logger.info(f"  - From epoch: {ckpt_meta.get('epoch', 'unknown')}")
        logger.info(f"  - Global step: {ckpt_meta.get('global_step', 'unknown')}")
    else:
        logger.warning("Stage 3 checkpoint not provided! Training from scratch.")
    ...
    # Create trainer (reuse Stage3Trainer)
    logger.info("\n" + "=" * 80)
