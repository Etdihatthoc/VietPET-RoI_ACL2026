...
def build_stage4_model(config: Dict[str, Any]) -> Tuple[HiRRA, Dict[str, Any]]:
    ...
    load_stage3_roi = model_cfg.get("load_stage3_roi", True)
    ...
    if merge_lora_from_stage2:
        ...
        if load_stage3_roi and not stage3_checkpoint:
    ...
    if merge_lora_from_stage2:
        ...
        if load_stage3_roi:
            ...
            # Stage 2 only (ablation): no Stage 3 ROI modules loaded
            print("\n[ModelBuilder] Loading Stage 2 checkpoint ONLY (no Stage 3 ROI modules)")
            print(f"  Stage 2 (global context): {stage2_checkpoint}")
            ...
            checkpoint_info = {
                "checkpoint": f"Stage2 only: {stage2_checkpoint}",
                "epoch": stage2_ckpt.get("epoch", "unknown"),
                "global_step": stage2_ckpt.get("global_step", "unknown"),
                "missing_keys": len(incompatible_s2.missing_keys),
                "unexpected_keys": len(incompatible_s2.unexpected_keys),
            }
